...
public final class SequenceSketch implements Serializable
{
			...
			FrequencyCounts kmerFilter, boolean weighted, boolean useAlignment)
	{
		...
		this.id = seq.getId();
		//FIXME Carlos
		//this.mainHashes = new MinHashSketch(seq.getSequenceString(), kmerSize, numHashes, kmerFilter, weighted);
		this.mainHashes = new MinHashSketch(seq.getSequenceString().replaceAll("[atcgn]", "N"), kmerSize, numHashes, kmerFilter, weighted);
		
		if (useAlignment)
		{
			this.orderedHashes = null;
			this.alignmentSketches = new MinHashBitSequenceSubSketches(seq.getSequenceString(), BIT_KMER_SIZE, SUBSEQUENCE_SIZE, BIT_SKETCH_SIZE);
		}
		else
		{
			//FIXME Carlos
			//this.orderedHashes = new OrderedNGramHashes(seq.getSequenceString(), orderedKmerSize);
			this.orderedHashes = new OrderedNGramHashes(seq.getSequenceString().toUpperCase(), orderedKmerSize);
